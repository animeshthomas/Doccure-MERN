import React, { useState, useEffect } from 'react';
import doctorImg from '../../assets/images/doctor-img02.png';
import starIcon from '../../assets/images/Star.png';
import DoctorsAbout from './DoctorsAbout';
import Feedback from './Feedback';
import SidePanel from './SidePanel';
import { useParams } from "react-router-dom";

const DoctorsDetails = () => {
  const [doctorDetails, setDoctorDetails] = useState({});
  const [tab, setTab] = useState('about');
  const { id } = useParams(); // Get the doctor's ID from the URL

  useEffect(() => {
    const fetchDoctorDetails = async () => {
      try {
        const response = await fetch(`http://localhost:500/api/v1/doctors/${id}`);
        const data = await response.json();
        setDoctorDetails(data.data);
      } catch (error) {
        console.error('Error fetching doctor details:', error);
      }
    };

    fetchDoctorDetails();
  }, [id]); // Fetch data when the ID change


  return (
    <section>
      <div className="max-w-[1170px] px-5 mx-auto">
        <div className="grid md:grid-cols-3 gap-[50px]">
          <div className="md:col-span-2">
            <div className="flex items-center gap-5">
              <figure className="max-w-[200px] max-h-[200px]">
                <img src={doctorDetails.photo || doctorImg} alt={doctorDetails.name} className="w-full" />
              </figure>

              <div>
                <span className="bg-[#CCF0F3] text-blue-400 py-1 px-6 lg:py-2 lg:px-6 text-[12px]
                  leading-4 lg:text-[16px] lg:leading-7 font-semibold rounded">
                  {doctorDetails.specialization || 'Surgeon'}
                </span>
                <h3 className="text-headingColor text-[22px] leading-9 mt-3 font-bold">
                  {doctorDetails.name || 'Muhibur Rahman'}
                </h3>
                <div className="Flex items-center gap-[6px]">
                  <span className="flex items-center gap-[6px] text-[14px] leading-5 lg:text-[16px] 
                  lg:leading-7 font-semibold text-headingColor">
                    <img src={starIcon} alt="" /> {doctorDetails.avgRating || '4.8'}
                  </span>
                  <span className="text-[14px] leading-5 lg:text-[16px] lg:leading-7 font-[400]
                  text-textColor">
                    ({doctorDetails.totalRating || '272'})
                  </span>
                </div>

                <p className="text__para text-[14px] leading-6 md:text-[15px] lg:max-w-[390px]">
                  {doctorDetails.description || 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Debitis sunt vel quisquam'}
                </p>
              </div>
            </div>

            <div className="mt-[50px] border-b border-solid border-[#0066ff34]">
              <button
                onClick={() => setTab('about')}
                className={`${tab === 'about' && 'border-b border-solid border-primaryColor'} py-2 px-5 mr-5 text-[16px] leading-7 text-headingColor font-semibold`}
              >
                About
              </button>

              <button
                onClick={() => setTab('feedback')}
                className={`${tab === 'feedback' && 'border-b border-solid border-primaryColor'} py-2 px-5 mr-5 text-[16px] leading-7 text-headingColor font-semibold`}
              >
                Feedback
              </button>
            </div>

            <div className="mt-[50px]">
              {tab === 'about' &&  <DoctorsAbout doctorDetails={doctorDetails} />}
              {tab === 'feedback' && <Feedback />}
            </div>
          </div>

          <div>
            <SidePanel />
          </div>
        </div>
      </div>
    </section>
  );
};

export default DoctorsDetails;
